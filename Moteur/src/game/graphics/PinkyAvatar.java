...
public class PinkyAvatar extends Avatar {
	...
	public BufferedImage[] change_animation() {
		...
		if (weak) {
			...
			animations = new BufferedImage[2];
			if (orientation >= 0 && orientation < 90) {
				animations[0] = sprite.getSubimage(455, 80, 16, 16);
				animations[1] = sprite.getSubimage(471, 80, 16, 16);
			} else if (orientation >= 90 && orientation < 180) {
				animations[0] = sprite.getSubimage(551, 81, 16, 16);
				animations[1] = sprite.getSubimage(567, 81, 16, 16);
			} else if (orientation >= 180 && orientation < 270) {
				animations[0] = sprite.getSubimage(487, 80, 16, 16);
				animations[1] = sprite.getSubimage(503, 80, 16, 16);
			} else {
				animations[0] = sprite.getSubimage(519, 80, 16, 16);
				animations[1] = sprite.getSubimage(536, 80, 16, 16);
			}
		}
		return animations;

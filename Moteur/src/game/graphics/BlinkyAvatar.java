...
public class BlinkyAvatar extends Avatar {
	...
	public BufferedImage[] change_animation() {
		...
		if (weak) {
			...
			animations = new BufferedImage[2];
			if (orientation >= 0 && orientation < 90) {
				animations[0] = sprite.getSubimage(455, 62, 16, 18);
				animations[1] = sprite.getSubimage(471, 62, 16, 18);
			} else if (orientation >= 90 && orientation < 180) {
				animations[0] = sprite.getSubimage(551, 63, 16, 18);
				animations[1] = sprite.getSubimage(567, 63, 16, 18);
			} else if (orientation >= 180 && orientation < 270) {
				animations[0] = sprite.getSubimage(487, 62, 16, 18);
				animations[1] = sprite.getSubimage(503, 62, 16, 18);
			} else {
				animations[0] = sprite.getSubimage(519, 62, 16, 18);
				animations[1] = sprite.getSubimage(536, 62, 16, 18);
			}
		}
		return animations;
